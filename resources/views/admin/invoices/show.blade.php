                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse($invoice->items as $item)
                                    <tr>
                                        ...
                                        <td class="px-6 py-3 font-semibold text-gray-900 text-right">₹{{ number_format((float) $item->total, 0) }}</td>
                                    </tr>
                                @empty
                    ...
                    <div class="px-6 py-4 border-t border-gray-100 flex justify-end">
                        <div class="w-full max-w-xs space-y-2 text-sm">
                            <div class="flex justify-between"><span class="text-gray-500">Subtotal</span><span class="font-semibold">₹{{ number_format((float) $invoice->subtotal, 0) }}</span></div>
                            <div class="flex justify-between"><span class="text-gray-500">Discount</span><span class="font-semibold text-red-600">− ₹{{ number_format((float) $invoice->discount_total, 0) }}</span></div>
                            <div class="flex justify-between"><span class="text-gray-500">GST</span><span class="font-semibold">₹{{ number_format((float) $invoice->gst_total, 0) }}</span></div>
                            <div class="flex justify-between border-t border-gray-200 pt-2"><span class="font-semibold text-gray-900">Grand Total</span><span class="font-bold text-brand-700 text-base">₹{{ number_format((float) $invoice->grand_total, 0) }}</span></div>
                            <div class="flex justify-between"><span class="text-gray-500">Paid</span><span class="font-semibold text-green-600">₹{{ number_format((float) $invoice->amount_paid, 0) }}</span></div>
                            <div class="flex justify-between"><span class="text-gray-500">Balance Due</span><span class="font-bold text-gray-900">₹{{ number_format($invoice->balanceDue(), 0) }}</span></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div class="space-y-2 mb-4">
                            @foreach($invoice->payments as $payment)
                                <div class="flex items-center justify-between rounded-xl bg-gray-50 border border-gray-100 px-4 py-2.5 text-sm">
                                    <div>
                                        <span class="font-semibold text-gray-900">₹{{ number_format((float) $payment->amount, 0) }}</span>
                    ...
                    @if(! $invoice->isCancelled() && $invoice->balanceDue() > 0)
                        <form action="{{ route('admin.invoices.payments.store', $invoice) }}" method="POST" class="flex flex-wrap items-end gap-3">
                            @csrf
                            <div>
                                <label class="block text-xs font-medium text-gray-500 mb-1">Amount (max ₹{{ number_format($invoice->balanceDue(), 0) }})</label>
                                <input type="number" name="amount" step="1" min="1" max="{{ $invoice->balanceDue() }}" required
