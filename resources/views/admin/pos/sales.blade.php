...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Manage all counter sales, receipts, and retail tax invoices.</p>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
            <span class="text-xs font-semibold uppercase text-gray-400">Digital / UPI Sales</span>
            <p class="text-2xl font-extrabold text-indigo-600 mt-1">₹{{ number_format($upiSales, 2) }}</p>
            <p class="text-xs text-gray-500 mt-1">Bank & QR collections</p>
        </div>
    </div>
    ...
    <form method="GET" action="{{ route('admin.pos.sales') }}" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 space-y-3">
        <div class="flex flex-wrap items-center gap-3">
            ...
            @if(request()->anyFilled(['q', 'payment_method', 'status', 'from', 'to']))
                    ...
                    @forelse($sales as $sale)
                        <tr class="hover:bg-gray-50/75 transition-colors">
                            ...
                            <td class="py-3.5 px-4 text-xs">
                                <span class="font-medium text-gray-800">{{ \App\Models\PosSale::PAYMENT_METHODS[$sale->payment_method] ?? $sale->payment_method }}</span>
                            </td>
                            ...
                            <td class="py-3.5 px-4 text-right">
                                <div class="inline-flex items-center gap-2">
                                    ...
                                    <a href="{{ route('admin.pos.invoice', $sale) }}" target="_blank" title="A4 Tax Invoice" class="p-1.5 text-gray-500 hover:text-green-600 hover:bg-gray-100 rounded-lg transition">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                    </a>
                                    @if($sale->isCompleted())
                                        <form method="POST" action="{{ route('admin.pos.sales.cancel', $sale) }}" onsubmit="return confirm('Cancel this sale? Deducted inventory will be reversed back to stock.');" class="inline">
                                            @csrf
