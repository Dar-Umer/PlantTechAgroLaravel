    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
            ...
            @if($sale->customer_gstin)
                <p class="text-xs font-mono text-gray-700 mt-0.5">GSTIN: {{ $sale->customer_gstin }}</p>
            @endif
        </div>

        <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
            <span class="text-xs font-semibold uppercase text-gray-400">Payment Information</span>
            ...
            @if($sale->payment_method === 'cash')
                <p class="text-xs text-gray-500 mt-0.5">Tendered: ₹{{ number_format($sale->amount_tendered, 2) }}</p>
                <p class="text-xs text-emerald-600 font-semibold">Change: ₹{{ number_format($sale->change_amount, 2) }}</p>
            @endif
        </div>
            ...
            <tbody class="divide-y divide-gray-100 text-xs">
                @foreach($sale->items as $item)
                    <tr>
                        ...
                        <td class="py-3 px-4 font-mono text-gray-500">{{ $item->batch?->batch_number ?? 'Auto-FIFO Lot' }}</td>
                        ...
                        <td class="py-3 px-4 text-right">₹{{ number_format($item->tax_amount, 2) }}</td>
